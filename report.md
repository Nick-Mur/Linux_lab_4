...
**Приложить скриншот этого вывода.**
...
- **Как автоматически очищать Docker:** Вы можете настроить автоматическую очистку Docker с помощью `cron` или `systemd` таймеров. Это было подробно описано ранее.

- **Ограничение использования ресурсов Docker:** При запуске контейнеров можно использовать параметры `--memory`, `--cpus`, `--storage-opt`, чтобы ограничить использование ресурсов.

---

## Скриншоты

**[Вставьте сюда скриншоты работы контейнера с `aafire` и проверки соединения с помощью `ping`.]**

---

**Примечание:** Всегда следите за тем, чтобы не удалять нужные данные при очистке Docker. Рекомендуется регулярно проверять использование дискового пространства и управлять ресурсами Docker для поддержания системы в оптимальном состоянии.

---

# Ответы на дополнительные вопросы

### Как очистить Docker после работы с ним?

- Остановите и удалите ненужные контейнеры:
